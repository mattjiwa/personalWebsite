...
Let's briefly review the parameters. We find an $\alpha$ of 0.7, capturing the typical diminishing returns of the value function, while the $\lambda$ around 2.35 reflects the expected loss-aversion. The $\theta_1$ value just above 1 indicates that contestants initially set their reference point just above the current offer, while $\theta_2$ and $\theta_3$ suggest that the reference point is (partially) drawn towards the expected value of recent rounds and of the game overall. Finally, contestants estimate $\rho$ to be higher than it truly is, meaning they expect the offers to improve more quickly than they do.
